...
namespace Fluke900Link.Helpers
{
    ...
    public static class FileHelper
    {
        ...
        public static FileLocations? GetFileLocation(string fileName)
        {
            ...
            else if (fileName.ToUpper().EndsWith(":PC")) return Fluke900Link.FileLocations.LocalComputer;
            return null;
        }
        ...
        public static bool ValidateFilename(string filename)
        {
            ...
            {
                sb.AppendLine("Filename must be less than 12 characters.");
                isValid = false;
            }
            //must start with a alpha character
            if (!Regex.Match(filename, @"^[a-zA-Z].*").Success)
            {
                sb.AppendLine("Filename must start with a letter.");
                isValid = false;
            }

            if (!isValid)
            {
                MessageBox.Show(sb.ToString(), "Filename Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            return isValid;
        }
        ...
        {
            string templateFilename = Path.Combine(Properties.Settings.Default.DefaultFilesDirectory, "_Templates", extension.Replace(".", "") + ".template");
